...
export class MyPoints {
  ...
  get userLevel() {
    ...
    if (pts <= 300) return { level: 2, label: 'Initi̩', min: 100, max: 300, progress: Math.round(((pts - 100) / 200) * 100) };
    ...
    return { level: 5, label: 'L̩gende', min: 1000, max: 2000, progress: Math.min(100, Math.round(((pts - 1000) / 1000) * 100)) };
  }
